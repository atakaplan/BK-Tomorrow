  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      const { data } = await axios.get(
        `https://corsproxy.io/?${encodeURIComponent(`https://newsapi.org/v2/everything?q=${query}&page=${page}&apiKey=a0c3d50dc320406c87c6732b38f348e4`)}`,
      );
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {articles.map((article, index) => (
          <Link key={index} to={`/detail/${encodeURIComponent(article.title)}`} state={{ article }}>
